...
def classify_method(dfs: Dataset, original_dfs: Dataset, method_class, is_test: bool, **kwargs):
    ...
    if CONFIG["use_f1"]:
        return f1_score( original_dfs[test_type][CONFIG['pred_col']], clf.predict( dfs[test_type] ) )
...
def classify_mlp(od_matrices: Dataset, original_dfs: Dataset, is_test: bool, **kwargs):
    ...
    del kwargs["hidden"]
    return classify_method( od_matrices, original_dfs, MLPClassifier, is_test, warm_start=True, hidden_layer_sizes=hidden_layer_sizes, early_stopping=True, learning_rate='adaptive', **kwargs )
